...
function renderCart() {
    ...
    const shippingBtn = document.querySelector('input[type="submit"]');
    ...
    for (let i = 0; i < cart.length; i++) {
		...
		let sizes;
		switch(item.id){
			case 4: sizes = ["field", "forest"];
					break;
			case 5: sizes = ["undyed", "red", "orange", "yellow", "green", "blue", "purple", "black", "brown", "suprise"];
					break;
			case 6: sizes = ["donate"];
					break;
...
row.appendChild(qtySel);
		...
		if (item.id == 6) remBtn.setAttribute("id", "remDonation");
